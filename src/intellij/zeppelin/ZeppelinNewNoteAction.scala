...
class ZeppelinNewNoteAction extends ZeppelinAction {
  ...
  override def actionPerformed(anActionEvent: AnActionEvent): Unit = {
    ...
    ZeppelinApi.createNotebook(name).map { notebook =>
        show(s"Created new Zeppelin notebook '$name': ${notebook.id}")

        runWriteAction(anActionEvent){ _ =>
          insertBefore(currentEditor(anActionEvent), notebook)
        }
  ...
  def insertBefore(editor: Editor, notebook: Notebook): Unit = {
    ...
    val message = notebook.notebookHeader
